...
/*filename df1 'C:\Users\kqin\Desktop\kqy\New_Wireless_Fixed.txt';

data ds1;
infile df1
DSD DLM=' ';

attrib acctno informat=$13. format= $13.
actdt informat=mmddyy10. format= mmddyy10.
deactdt informat=mmddyy10. format= mmddyy10.
deactreason informat= $4. format= $4.
goodcredit informat=1. format= 1. 
rateplan informat=1. format= 1.
dealertype informat=$2. format= $2. 
AGE informat=2. format=2. 
Province informat=$2. format=$2. 
sales informat=dollar10.2 format=dollar10.2;

input @1 acctno  @15 actdt  @26 deactdt  @41 deactreason  @53 goodcredit  @62 rateplan  @65 dealertype 
@74 AGE  @80 Province sales dollar10.2;
run;*/
...
/*version 3: delimited ds (proc import is better to deal with it)*/
Proc import datafile='C:\Users\Jesus kid\Desktop\kqylib\New_Wireless_Pipe.txt'
            Out=work.df1
            DBMS=DLM
            REPLACE;
...
Run;
/*the acctno is read into a numeric format (but its too big, its represented by exponetial form)*/

data df1;
retain acc Actdt Deactdt DeactReason GoodCredit RatePlan DealerType AGE Province Sales;
*retain the order of the vars (acc is the new var);
set df1;
acc= put(acctno, best13.); 
*put (var_name, informat of numeric) numeric to character;
*input (var_name, informat of char) character to numeric;
drop acctno;

format acc $13. actdt mmddyy10. deactdt mmddyy10. deactreason $4. goodcredit 1. rateplan 1. dealertype $2.
AGE 2. Province $2. sales dollar8.2;
run;

proc print data= df1 (obs=10);
run;
...
/*test out input(), put()*/
data df2 (drop = tempacc tempsales tempactdt);
retain acc Actdt Deactdt DeactReason GoodCredit RatePlan DealerType AGE Province Sales;
set df1 (rename=(acc=tempacc sales=tempsales actdt= tempactdt));
format acc comma17. ;*specify the output format for new acc;
acc=input(tempacc, 13.); *input(, infomat for tempacc);
sales=put(tempsales, comma9.); *put() always gives char, put(, format of the char u want to output with using $);
actdt=put(tempactdt,date9.); * if want convert date format, use put(, desired format);
run;



data df3 (drop = tempsales tempacc);
retain acc Actdt Deactdt DeactReason GoodCredit RatePlan DealerType AGE Province Sales;
set df2 (rename=( sales=tempsales acc=tempacc));
format sales comma9.1 ;*specify the output format for new acc
(if dont specify the format, sales will be still be converted to numeric, but without comma by default convertion);
sales=input(tempsales, comma9.); * to convert char to numeric (informat of the source var, dont need $);
acc=put(tempacc, 13.);  *convert comma17. to format char with length 13.(no need for $);
run;




•	PUT( , returned format) always creates character variables
...
B  PUT( , always n. (eg: 9. Will convert to char value with length of 9)) converts numeric variable to a character variable with numeric value.
...
A  PUT(name, $10.);	char, char format	‘Richard’	char always	‘Richard   ’  length 10 but char
...
/*example for converting date format*/
data forcast;
set ds;
format mmyy monyy7.;
mmyy = input(put(deactdt,monyy7.), monyy7.);
run;
